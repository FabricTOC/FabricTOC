...
| :---: |
| Authenticating data using private keys and public keys. Mary's private key is used to sign the original document with a unique signature (`X13vRZQql41`). Any holder of Mary's public key can verify that this signature could only have been generated by the holder of Mary's private key.  Moreover, if the document is modified with, the signature will be incorrect for the tempered text, as only Mary can generate the correctly updated signature. Finally, generating the correct key for the tampered text is incredibly unlikely -- the larger the signature is, the more mathematically unlikely it is to do this - that's the nature of cryptography. |
