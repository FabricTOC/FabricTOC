...
For authentication, Mary uses her private key to attach a small, fixed size, unique data signature to an original document of any size. The signature is generated by combining the document contents with the private key in a process called **hashing**. This signed document can now be sent to any other principal in the network -- and they can verify that it has not been tampered with, using Mary's public key. The consumer of the transmitted document is confident that they have an authentic copy of the original document from Mary.
...
It's also worthing noting how hard it is to try to tamper with the document while it's being transmitted. Note that if the original document is modified, the signature will no longer match, as only Mary can generate a correct signature. Or if an intermediary tries to generate the same signature as Mary would do, without Mary's private key it is incredibly unlikely -- that's the mathematical beauty of cryptography.
