...
**Authentication** ensures that information has not been tampered with since it was generated; information that might relate to the a principal's identity or information generated by a principal. For example, you might want to be sure you're communicating with the real Mary Mitchell rather than an impersonator; or if Mary has generated some information, then you want to be sure that it's not changed since Mary generated it. In both cases, authenticity is the watch-word.  

**Encryption**, on other hand, is quite different from authentication -- it enables the private transmission of information between principals; encrypted information cannot be read by anyone other than its intended recipients, when it is decrypted by them, and no-one else.  
