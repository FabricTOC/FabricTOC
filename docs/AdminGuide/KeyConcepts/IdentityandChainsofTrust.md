...
You may find it helpful to see how local and global MSPs are used by considering an a blockchain administrator who wants to install and instantiate a smart contract, and this is schematically shown in the [diagram above](MSP2).

An administrator `B` connects to the peer with an identity issued by `RCA1`. When the `B` tries to install a smart contract on the peer, the peer checks it's **local MSP**, `ORG1.MSP` to verify that the administrators identity is indeed a member of `ORG1`. A successful verification will allow the operation to complete successfully. Subsequently, the `B` wishes to instantiate the smart contract on the channel, but as this is a channel operation, which all organization must agree to,  this the peer must check the **global MSP** before it can sucessfully complete this command.  (Other things must happen too, but we ignore those right now.)

You can see that the channel and the ledger are really logical constructs that exist at the channel level, but are instantiated and managed the peers that are the members of the channel.  Whether it's the channel's policy, or the ledger itself, channel level artefacts are instantiated and managed by the peers, but with reference to the channel policy and therefore global MSPs.
