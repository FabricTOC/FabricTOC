...
An administrator `B` connects to the peer with an identity issued by `RCA1`. When the `B` tries to install a smart contract on the peer, the peer checks its **local MSP**, `ORG1.MSP`, to verify that the identity is indeed a member of `ORG1`. A successful verification will allow the install command to complete successfully. Subsequently, `B` wishes to instantiate the smart contract on the channel. Because this is a channel operation, all organization must agree to it, so the peer must check the **global MSP** in the channel policy before it can successfully complete this command.  (Other things must happen too, but we ignore those right now.)

You can see that the channel and the ledger are really logical constructs when they are defined at the channel level. It is only when they are instantiated and managed by the peers in the channel that they become physical entities on the local file system. It's really important to understand the why things like global MSPs, channel policies and even the ledger itself is defined at the channel level, but instantiated and managed by the peers in the different organizations. This split between global and local resources fundamentally stems from the fact that there are multiple organizations who form the network with their resources, but who also need to share these resources and agree their operation.
