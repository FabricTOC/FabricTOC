...
Whenever a peer connects to a channel in a blockchain network, **it's this identity that is used to determine the peer's organization**. The mapping of identity to organization is provided by a component called a [Membership Service Provider](../Key Concepts/Membership.md) (MSP), and is central to understanding how peers get assigned to organizations and subsequently gain particular access to blockchain resources.  We'll learn more about peers and access control later in this topic, and there's a entire topic on MSPs and access control policies in this guide. But for now, think of an MSP as providing linkage between an individual identity and a particular organization in a blockchain network.
