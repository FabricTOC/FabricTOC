...
We've seen that peers form a blockchain network, hosting ledgers and smart contracts which can be queried and updated by peer-connected applications. However, the mechanism by which applications and peers interact with each other to ensure that every peer's ledger is kept consistent is mediated by nodes called *orderers*, and it's these nodes to which we now turn our attention.

Applications that want to update the ledger are involved in a 3-step process, which ensures that the all the peers in a blockchain network keep their ledgers consistent with each other. In the first step, applications work with a subset of *endorsing peers*, each of which their individual stamp of approval to a proposed ledger update, but without the applying the proposed update to their copy of the ledger. In the second step, these individually proposed updates are collected together into blocks by ordering nodes in preparation for them to be checked for consistency. In the final step, the orderers distribute these blocks of transaction back to every peer hosting a copy of the ledger. At each peer, transaction proposals from different peers are cross-checked with each other for validity before being applied to that peer's copy of the ledger.

As you can see, orderer nodes are central to this process -- so let's investigate in a little more detail how applications and peers use orderers to generate ledger updates that can be consistently applied to a distributed, replicated ledger.

### Phase 1: Applications propose transactions to the ledger using endorsing peers

The first phase of the transaction workflow does not involve the orderer.  Applications generate transaction proposals which they send to the required set of peers for endorsement on a particular channel. Each peer independently executes a transaction proposal for a smart contract to generate a single transaction response which they return to the application. Once the application has received all transaction responses from all peers, the first phase of the transaction flow is complete. Let's examine this phase in a little more detail.
...
Initially, a set of peers are chosen by the application to generate a set of proposed ledger updates. Which peers are chosen by the application? Well, that depends on the *smart contract endorsement policy*, which define the set of organizations that need to be happy with a proposed ledger change before it can be accepted by the network.  This is literally what it means to achieve consensus -- everyone must be happy with the proposed ledger change before it can be applied.

However -- it's important to remember that during phase 1, any individual peer cannot update its copy of the ledger. That's because each peer is operating independently, and applying an update unilaterally could easily result in different peers applying different ledger updates, either because the ledger changes calculated by the smart contract are different, or because every organization is not happy with the change, or because the order in which transactions are applied could be different on different peers.  In summary phase 1 sees each endorsing peer generate a transaction response which describes the change that it's organization would be happy to see applied to the ledger -- *but the peer does not apply it to the ledger in phase 1*.  

We'll see in phase 3 how the all the transaction proposal responses from different peers are validated together in such a way as to make sure they would provide an agreed and consistent ledger update, and how the orderer is central to that process.  But for now, let's note that phase 1 is only concerned with an application asking organizations endorsing peers agree that each would be happy to apply a particular transaction proposal to the ledger, without applying it.
...
Phase 1 ends when the application receives endorsed responses from all the peers to which it sent transaction proposals.  We note that different peers can return different and inconsistent transaction responses to the application *for the same transaction proposal*. That's because the transaction resulted can be different when it is executed on different peers -- and the term used to describe this kind of failure is *non-determinism*. Non-determinism is the enemy of Smart contracts, and if it occurs, there is serious problem with the proposed transaction -- it will not generate a consistent result and therefore cannot be applied to the ledger.  Of course, individual peers cannot know that their result is non-deterministic -- we require transaction responses to be compared before non-determinism can be detected.  (Strictly speaking, even this is not enough, but we defer this discussion to the transaction topic, where non-determinism is discussed in detail.)
...
The second phase of the transaction worklow is packaging. The orderer is pivotal to this process -- it receives endorsed transaction proposal responses from applications, and packages them into blocks ready for distribution back to all peers connected to the orderer, not just endorsing peers.
...
In summary, orderers are responsible for the simple but vital processes of collecting proposed transaction updates, ordering them, packaging them into blocks and distributing these blocks to peers for subsequent validation.  
